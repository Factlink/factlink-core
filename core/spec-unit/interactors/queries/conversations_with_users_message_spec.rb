...
describe Queries::ConversationsWithUsersMessage do
  ...
  describe '#all_recipients_by_ids' do
    it 'should return the recipients indexed by ids' do
      mocklist = [
        mock('foo', id: 1),
        mock('bar', id: 2)
      ]
  ...
  describe '#wrap_with_ids' do
    it 'wraps its input in a hash, with as index the id of the objects in the array' do
      ...
      query = described_class.new(mock, current_user: mock)
  ...
  describe '#call' do
    it 'should call the three other queries' do
      ...
      query.stub(:query).with(:conversations_list, user1.id.to_s).
        and_return(conversations)
      query.stub(:all_recipients_by_ids).with([conversation10, conversation20]).and_return(1 => user1, 2 => user2)
      query.stub(:query).with(:last_message_for_conversation, conversation10).
        and_return(message15)
      query.stub(:query).with(:last_message_for_conversation, conversation20).
