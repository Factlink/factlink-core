...
# sort is to make sure the file order is always the same
CURRENT_SHASUM=`shasum -b $(ls config/locales/* | sort) | shasum -b | perl -pe 's/\s*\*-\s*//'`
...
if [ "$CURRENT_SHASUM" != "$RECORDED_SHASUM" ]; then
  ...
  LC_ALL=C shasum -b $(ls config/locales/* | sort)
