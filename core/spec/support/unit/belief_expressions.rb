module BeliefExpressions
  ...
  def add_supporting_comment(graph_user, fact)
    comment = Pavlov.command(:'create_comment', fact_id: fact.id.to_i, type: 'believes', content: 'comment', user_id: graph_user.user.id.to_s)
    something_happened
    comment
  end

  def add_weakening_comment(graph_user, fact)
    comment = Pavlov.command(:'create_comment', fact_id: fact.id.to_i, type: 'disbelieves', content: 'comment', user_id: graph_user.user.id.to_s)
    something_happened
    comment
  end

  def believes_comment(graph_user, comment)
    Pavlov.command(:'comments/set_opinion', comment_id: comment.id.to_s, opinion: 'believes', graph_user: graph_user)
  end

  def disbelieves_comment(graph_user, comment)
    Pavlov.command(:'comments/set_opinion', comment_id: comment.id.to_s, opinion: 'disbelieves', graph_user: graph_user)
  end
  ...
  def opinion?(fact)
    possible_reset
    opinion = FactGraph.new.opinion_for_fact(fact)
    opinion.should
  end
