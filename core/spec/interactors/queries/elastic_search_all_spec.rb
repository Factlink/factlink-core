...
require File.expand_path('../../../../app/interactors/queries/elastic_search_all.rb', __FILE__)
    ...
    ['user', 'topic', 'factdata'].each do |type|
      it "correctly with return value of #{type} class" do
        ...
        HTTParty.should_receive(:get).
          with("http://#{base_url}/_search?q=#{wildcard_keywords}&from=0&size=20&default_operator=AND").
          and_return(results)
