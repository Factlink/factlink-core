...
class JsLibUrl
  ...
  class Builder
    ...
    def url_for username
      ::JsLibUrl.new Username.new(username), @salt, @base_url
    end
    def url_from_string url
      ::JsLibUrl.new username_from_string(url), @salt, @base_url
    end

    def username_from_string url
      encoded_username = JsLibUrl.username_from_url(url,@base_url, @salt)
    ...
    def nginx_line
      ...
      url = JsLibUrl.new(fake_username, @salt, @base_url).to_s
