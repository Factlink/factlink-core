class ChannelsController < ApplicationController
  ...
  def activities
    ...
    if @channel.type == "stream"
      # TODO Tom, clean this.
      @activities = @user.graph_user.notifications.below('inf', count: 10, reversed: true ).keep_if { |a| a && a.still_valid? }.map { |activity| Notifications::Activity.for(activity: activity, view: view_context) }
    else
