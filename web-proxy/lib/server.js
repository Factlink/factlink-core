...
function getServer(config_path) {
  ...
  config = require('./read_config').read_conf(config_path, fs, server.settings.env);

  var STATIC_URL  = config['static'].protocol + config['static'].hostname + ':' + config['static'].port;
  var API_URL     = config.core.protocol + config.core.hostname + ':' + config.core.port;
  var PROXY_URL   = config.proxy.protocol + config.proxy.hostname + ':' + config.proxy.port;
  var LIB_URL     = STATIC_URL + "/lib";
  var INTERNAL_PROXY_PORT = parseInt(config.proxy.internal_port, 10);
  
  server.INTERNAL_PROXY_PORT = INTERNAL_PROXY_PORT;
